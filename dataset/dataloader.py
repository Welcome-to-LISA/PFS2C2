...
class Dataset(data.Dataset):
    def __init__(self, args):
        ...
        io.savemat('sp_matrix_p3.mat', {'data':   self.sp_matrix})
        ...
        self.InvPSF = self.create_inverse_PSF(shape=(self.args.scale_factor, self.args.scale_factor), sigma=self.args.sigma)

        data_folder = os.path.join(self.args.default_datapath, args.data_name)

        print("sad", data_folder)
        ...
        for i in range(len(self.imgpath_list)):
            # self.img_list.append(io.loadmat(self.imgpath_list[i])['data'])
            # self.img_list.append(io.loadmat(self.imgpath_list[i])['REF'])
            self.img_list.append(mat73.loadmat(self.imgpath_list[i])['data'])
    ...
    def matlab_style_gauss2D(self, shape=(3, 3), sigma=2):
        ...
        # obtain the sepctral response stored in  data//spectral_response//houston.xls
    ...
    def __getitem__(self, index):
        ...
        io.savemat(r'J:\6note\7papers\Jia_汇总\JRS\code\sa\paviau_4_hsi.mat', {'data': img_lr})
        io.savemat(r'J:\6note\7papers\Jia_汇总\JRS\code\sa\paviau_4_msi.mat', {'data': img_msi})
