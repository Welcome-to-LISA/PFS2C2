...
def Spectral_upsample(args, msi_channels, hsi_channels, init_type='kaiming', init_gain=0.02, initializer=False):
    ...
    print("uuu",hsi_channels)
    print("sss",msi_channels)
...
class spectral_upsample(nn.Module):
    def __init__(self, msi_channels, hsi_channels, need_clamp=False):
        ...
        print(type(msi_channels),msi_channels)
        print(type(self.num_ups), self.num_ups)


        for i in range(1, self.num_ups + 1):
            self.layers += [
                nn.Conv2d(msi_channels * (2 ** (i - 1)), msi_channels * (2 ** i), kernel_size=1, stride=1, padding=0),
                nn.LeakyReLU(0.2, True)]

        # self.layers += [nn.Conv2d(int(msi_channels * (2 ** self.num_ups)), hsi_channels, kernel_size=1, stride=1, padding=0)]
