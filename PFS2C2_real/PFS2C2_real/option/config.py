...
### common parameters
parser.add_argument('--scale_factor', type=int, default=3, help='scale_factor, set to 5 and 8 in our experiment')
...
parser.add_argument('--data_name', type=str, default="ZY1E", help='name of your data and spectral response. houston18 is given as a example')
...
parser.add_argument("--epoch_stage1", type=int, default=10000, help='total epoch')
parser.add_argument("--decay_begin_epoch_stage1", type=int, default=5000, help='epoch which begins to decay,so the lr is 1e-3 in the first 10000 epochs and then it decays from 10000th epoch to 20000th epoch. When 20000 epochs are finished, the lr decays to 0')

parser.add_argument("--lr_stage2", type=float, default=1e-3, help='learning rate')
parser.add_argument("--epoch_stage2", type=int, default=10000, help='total epoch')
parser.add_argument("--decay_begin_epoch_stage2", type=int, default=5000, help='epoch which begins to decay,so the lr is 1e-3 in the first 10000 epochs and then it decays from 10000th epoch to 20000th epoch. When 20000 epochs are finished, the lr decays to 0')


#the second stage
parser.add_argument("--lr_stage3", type=float, default=1e-3, help='learning rate')
parser.add_argument("--epoch_stage3", type=int, default=9000, help='total epoch')
parser.add_argument("--decay_begin_epoch_stage3", type=int, default=60000, help='epoch which begins to decay,so the lr is 1e-3 in the first 20000 epochs and then it decays from 20000th epoch to 30000th epoch. When 30000 epochs are finished, the lr decays to 0')
